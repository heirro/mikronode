...
module.exports = (function() {
	...
	var debugSocket = dbg('mikronode-ng:socket');
	var debugSocketData = dbg('mikronode-ng:socket:data');
	var debugLogin = dbg('mikronode-ng:login');
	var debugSentence = dbg('mikronode-ng:sentence');
	var debugConnection = dbg('mikronode-ng:connection');
	var debugPromise = dbg('mikronode-ng:promise');
	...
	Connection.prototype.connect = function connect(callBack) {
		...
		_(this).socket.on('error', function(a) {
			try {
				...
				console.log('Error in listener "error":', error);
			}
		});
	...
	Connection.prototype.loginStateMachine = function loginStateMachine() {
		...
		//GSB START Support only post RouterOS 6.43 and above
		//lc.write('/login');
		...
		//GSB END Support only post RouterOS 6.43 and above
		...
		lc.on('done', function loginDone(data) {
			...
			case 'Waiting for challenge':
				...
				var challenge = new Buffer(_(_this).password.length + 17);
				challenge.write(emptyString + _(_this).password, 0, 'utf8');
				challenge.write(parsed[0].ret, _(_this).password.length + 1, 'hex');

				var resp = "00" + crypto.createHash('MD5').update(challenge).digest("hex");

				_(_this).status = 'Sending Credentials';
				debugLogin('%s %s %s', _(_this).status, _(_this).user, resp);
				this.write('/login', {
					"=name" : _(_this).user,
					"=response" : resp
				});
